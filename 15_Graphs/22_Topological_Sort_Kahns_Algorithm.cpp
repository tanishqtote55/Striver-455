...
public:
    vector<int> topoSort(int V, vector<int> adj[]) {
        vector<int> inDegree(V, 0);  // Step 1: Calculate in-degree of each node
        ...
        // Step 4: Check for cycle (i.e., invalid topological sort)
        if (topoOrder.size() != V) {
            return {}; // Graph has a cycle, so topological sort is not possible
        }
